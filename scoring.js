$(document).ready(function() {
  ...
  function score_indicate() {
    ...
    let subject_points = [Number($('#national_language').val()),
      //文字列（教科の点数）を数値に変換 F  //id=national_languageフォームに入力されたvalue値の取得 A
      Number($('#english').val()),
    ...
    //id=sum_indicateの箇所にsum(教科の合計点)を出力。 B
    let average = sum / 5
  ...
  function judgement() {
    ...
    // id=declarationの箇所に次の要素を挿入する。 B
  };
  $('#national_language, #english, #mathematics, #science, #society').change(function() {
    //各教科の点数が変わることでfunc実行 F
    score_indicate();
  });
  $('#btn-evaluation').click(function() {
    //id=btn-evaluationボタンをクリックすることでfunc実行 F
